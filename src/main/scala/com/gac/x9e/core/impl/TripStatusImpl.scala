...
object TripStatusImpl extends TripStatus {
  ...
  private def mappingFunction(vin: String,
    ...
    if (state.hasTimedOut) {
      ...
      for {
        trip <- currentState
        timeoutTrip = endedTripSession(vin, trip, isTripEnded = true)
      } tripResult.append(timeoutTrip)
    } else {
      updateState(vin = vin, source = sourceData, state = state, tripResult = tripResult)
    }

    println(s"$vin has output ${tripResult.length} elems in this batch")
    tripResult.iterator
  }
   ...
   * @return TripSession
   */
  private def endedTripSession(vin: String, t: TripState, isTripEnded: Boolean): TripSession = {
    TripSession(
      vin = vin,
  ...
  private def updateState(vin: String,
    ...
    for {
      ...
      if (s._1.createTime.getTime - tripState.tripEndTime > tripGapDuration) {
        val endTrip = endedTripSession(vin, tripState, isTripEnded = true)
        tripResult.append(endTrip)
        state.update(initTripState(s._1)) // 初始化下一个行程
      } else {
        // 行程进行中, 更新 state
        val updatedTripState = updateTripState(s._1, tripState)
        ...
        for {
          ...
          tripResult.append(endedTripSession(vin, currentState, isTripEnded = false))
        }
      }
      state.setTimeoutTimestamp(state.getCurrentWatermarkMs() + timeoutDuration) // 设置超时时间
    }
